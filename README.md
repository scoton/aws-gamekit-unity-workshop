...
9. A partir do arquivo `.csv` gerado no momento em que criou seu usuário para o AWs GameKit, preencha os campos `Access Key ID` e `Secret Access Key`. O seu número de conta de 9 dígitos deve ser automaticamente preenchido a seguir. Caso não tenha sido preenchido automaticamente, isso significa que pode ter havido algum erro nas etapas anteriores. Se estiver na Campus Party, é uma boa hora para pedir ajuda à equipe da AWS. Caso o número de 9 dígitos esteja aparecendo, clique no botão `Submit`.
